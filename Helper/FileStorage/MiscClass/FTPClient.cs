...
namespace FileStorage.MiscClass
{
    ...
    public class FTPClient
    {
        ...
        #endregion
        public string FolderTempGenerated()
        {
            string localPath = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
            return localPath;
        }
        public byte[] StorageDownload(string downloadFromLocation,string fileNameWithExt)
        {
          
            //2. check source
            string target= Path.Combine(downloadFromLocation, fileNameWithExt);
            if (target.Trim() == "")
            {
                throw (new ApplicationException("File not specified"));
            }
            ...
            string tempFileName = Guid.NewGuid().ToString()+Path.GetExtension(fileNameWithExt);
            ...
            {
                result = File.ReadAllBytes(tempFileDownload);
        ...
        public StorageReturnValue StorageDownload(string pathFileNameWithExt)
        {
            ...
            if (resultx != null)
            {
                ...
                result = result.SetStorageReturnValue(true, string.Empty,
                    new List<FileStorageAttachment>() { new FileStorageAttachment(filename, filename, resultx) });
            }
            else
            {
                string filename = Path.GetFileName(pathFileNameWithExt);
        ...
        /// <returns></returns>
        /// <remarks></remarks>
        public bool FtpDelete(string pathFileNameWithExt)
        {
